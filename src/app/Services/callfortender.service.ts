...
export class CallForTenderService {
  private baseUrl = 'http://localhost:8089/CallForTinder';
  checkNameExists: any;
  ...
  search(query: string): Observable<any[]> {
   ...
   uploadPDF(file: File, ID: number): Observable<any> {
    const formData: FormData = new FormData();
    formData.append('file', file, file.name);
    return this.httpClient.post<any>(`${this.baseUrl}/${ID}/upload-pdf`, formData);
  }
}
