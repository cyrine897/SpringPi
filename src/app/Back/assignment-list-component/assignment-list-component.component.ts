...
onSearchTextChanged(event: Event) {
  // Extract the search value from the event object
  const searchValue = (event.target as HTMLInputElement).value;
  
  // Call the original method with the extracted search value
  this.onSearchTextChangedImpl(searchValue);
}

onSearchTextChangedImpl(searchValue: string) {
